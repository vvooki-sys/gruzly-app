...
export default async function BrandBookPage() {
  ...
  const [project] = await getDb()`SELECT * FROM projects WHERE id = ${brandId}`;
  ...
  const assets = await getDb()`SELECT * FROM brand_assets WHERE project_id = ${brandId} AND type = 'logo' ORDER BY created_at ASC`;
