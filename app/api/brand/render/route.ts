...
async function renderHandler(req: NextRequest) {
  ...
  // No custom fonts — Manrope TTF on disk is a variable font, unsupported by Satori
  const makeImageResponse = (el: React.ReactElement, w: number, h: number) =>
    new ImageResponse(el, { width: w, height: h });
  ...
  if (layout.zones && Array.isArray(layout.zones) && layout.zones.length > 0) {
    ...
    const filename = `gruzly/${BRAND_ID}/precision-${Date.now()}.png`;
